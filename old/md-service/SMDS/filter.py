...
# $Id: Filter.py 18344 2010-06-22 18:56:38Z caglar $
# $URL: http://svn.planet-lab.org/svn/PLCAPI/trunk/PLC/Filter.py $
...
from SMDS.parameter import Parameter, Mixed, python_type
      ...
      for field, value in self.iteritems():
         ...
         if not modifiers['-']:
               if field not in self.fields:
                  raise MDInvalidArgument, "Invalid filter field '%s'" % field
         ...
         # sorting and clipping
               ...
               if field not in ('SORT','OFFSET','LIMIT'):
                  raise MDInvalidArgument, "Invalid filter, unknown sort and clip field %r"%field
               # sorting
               if field == 'SORT':
                  if not isinstance(value,(list,tuple,set)):
                     value=[value]
                  for field in value:
                     ...
                     if field[0] == '+':
                           ...
                           order = 'DESC'
                     if field not in self.fields:
                           raise MDInvalidArgument, "Invalid field %r in SORT filter"%field
