...
int main( int argc, char** argv ) {
   ...
   md_set_debug_level( SG_MAX_VERBOSITY );
   ...
   if( rc != 0 ) {
      SG_error("md_download_set_init rc = %d\n", rc );
      exit(1);
   }
   ...
   for( int i = 1; i < argc; i++ ) {
      ...
      SG_debug("adding %s to the download set\n", url );
      rc = md_download_set_add( &dlset, dlctx );
      if( rc != 0 ) {
         SG_error("md_download_set_add( %s ) rc = %d\n", url, rc );
         exit(1);
      }
   ...
   while( md_download_set_size( &dlset ) > 0 ) {
      ...
      SG_debug("Wait for %zu downloads to finish...\n", md_download_set_size( &dlset ) );
      
      rc = md_download_context_wait_any( &dlset, -1 );
      if( rc != 0 ) {
         SG_error("md_download_wait_any rc = %d\n", rc );
      }
      ...
      for( md_download_set_iterator itr = md_download_set_begin( &dlset ); itr != md_download_set_end( &dlset ); ) {
         ...
         if( md_download_context_finalized( dlctx ) ) {
            ...
            if( rc != 0 ) {
               SG_error("md_downlaod_context_get_effective_url rc = %d\n", rc );
               exit(1);
            }
            ...
            if( rc != 0 ) {
               SG_error("md_downlaod_set_clear rc = %d\n", url, rc );
               exit(1);
            }
      ...
      SG_debug("Download set now has %zu downloads remaining\n", md_download_set_size( &dlset ) );
   }
   
   // free the download set 
   SG_debug("%s", "freeing download set\n");
   rc = md_download_set_free( &dlset );
   if( rc != 0 ) {
      SG_error("md_download_set_free rc = %d\n", rc );
      exit(1);
   }
