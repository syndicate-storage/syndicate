...
# Get the library at: https://github.com/openid/python-openid
# Run this on port 8081 if VICCI is down
...
class ServerHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            print "POST query = %s" % str(self.query)
    ...
    def handleAllow(self, query):
        ...
        # right?
        print self.server.lastCheckIDRequest
        if 'yes' in query:
    ...
    def serverEndPoint(self, query):
        ...
        if "openid.identity" in query:
           self.user = query['openid.identity'].split('/')[-1]
           print self.user

        print request.mode
        print "handleCheckIDRequest?"
