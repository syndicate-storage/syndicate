...
int fs_core_init( struct fs_core* core, struct md_syndicate_conf* conf ) {
   ...
   int rc = fs_entry_init_dir( core, core->root, "/", conf->metadata_url, 1, SYS_USER, conf->volume, 0755, 4096, 0, 0 );
   if( rc != 0 ) {
...
static int fs_entry_init_data( struct fs_core* core, struct fs_entry* fent, int type, char const* name, char const* url, int64_t version, uid_t owner, gid_t volume, mode_t mode, off_t size, int64_t mtime_sec, int32_t mtime_nsec ) {
   struct timespec ts;
...
static int fs_entry_init_common( struct fs_core* core, struct fs_entry* fent, int type, char const* name, char const* url, int64_t version, uid_t owner, gid_t volume, mode_t mode, off_t size, int64_t mtime_sec, int32_t mtime_nsec) {

   memset( fent, 0, sizeof(struct fs_entry) );
   fs_entry_init_data( core, fent, type, name, url, version, owner, volume, mode, size, mtime_sec, mtime_nsec );
...
int fs_entry_init_file( struct fs_core* core, struct fs_entry* fent, char const* name, char const* url, int64_t version, uid_t owner, gid_t volume, mode_t mode, off_t size, int64_t mtime_sec, int32_t mtime_nsec ) {
   fs_entry_init_common( core, fent, FTYPE_FILE, name, url, version, owner, volume, mode, size, mtime_sec, mtime_nsec );
...
int fs_entry_init_dir( struct fs_core* core, struct fs_entry* fent, char const* name, char const* url, int64_t version, uid_t owner, gid_t volume, mode_t mode, off_t size, int64_t mtime_sec, int32_t mtime_nsec ) {
   fs_entry_init_common( core, fent, FTYPE_DIR, name, url, version, owner, volume, mode, size, mtime_sec, mtime_nsec );
...
int fs_entry_dup( struct fs_core* core, struct fs_entry* fent, struct fs_entry* src ) {
   fs_entry_init_common( core, fent, src->ftype, src->name, src->url, src->version, src->owner, src->volume, src->mode, src->size, src->mtime_sec, src->mtime_nsec );
...
int fs_entry_init_md( struct fs_core* core, struct fs_entry* fent, struct md_entry* ent ) {
   ...
   if( ent->type == MD_ENTRY_DIR ) {
      // this is a directory
      fs_entry_init_dir( core, fent, basename, ent->url, ent->version, ent->owner, ent->volume, ent->mode, ent->size, ent->mtime_sec, ent->mtime_nsec );
   }
   else {
      // this is a file
      fs_entry_init_file( core, fent, basename, ent->url, ent->version, ent->owner, ent->volume, ent->mode, ent->size, ent->mtime_sec, ent->mtime_nsec );
   }
