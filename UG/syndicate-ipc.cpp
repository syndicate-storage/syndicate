...
public:
    ...
    void start() {
        ...
        socket_.async_read_some(boost::asio::buffer(data_in_, max_length),
        ...
        if (!error) {
            ...
            while(bytes_remain > 0) {
                if(stage_ == stage_read_header) {
                    if(bytes_remain >= max_header_length - header_offset_) {
                        int readSize = max_header_length - header_offset_;
                        ...
                        // parse header
                        op_code_ = getIntFromBytes(header_);
                        //dbprintf("hdr opcode : %d\n", op_code_);
                        total_msg_size_ = getIntFromBytes(header_ + 4);
                        //dbprintf("hdr msg_size : %d\n", total_msg_size_);
                        num_messages_ = getIntFromBytes(header_ + 8);
...
private:
    void handle_protocol() {
        ...
        case OP_GET_STAT:
            {
                ...
                readPath(message_, path, &bytes_ptr);
		...
		if(returncode == 0) {
		    toWriteSize += 4 + SIZE_IPCSTAT;
                ...
                data_out_ = new char[toWriteSize];
                char* outBuffer = data_out_;
                char* bufferNext;

                writeHeader(outBuffer, op_code_, returncode, 4 + SIZE_IPCSTAT, 1, &bufferNext);
		if(returncode == 0) {
                ...
                boost::asio::async_write(socket_,
                    boost::asio::buffer(data_out_, toWriteSize),
		...
		///////////
                ...
                writeHeader(outBuffer, op_code_, returncode, 0, 0, &bufferNext);
		...
		// readdir
		if(rc == 0) {
		    logmsg( SYNDICATEFS_DATA->logfile, "syndicateipc_readdir( %s )\n", path );
		...
		// closedir
		if(rc == 0) {
		    logmsg( SYNDICATEFS_DATA->logfile, "syndicateipc_releasedir( %s )\n", path );
   
		    SYNDICATEFS_DATA->stats->enter( STAT_RELEASEDIR );
   
		    rc = fs_entry_closedir( SYNDICATEFS_DATA->core, fdh );
		...
		totalMessageSize += 4 * numOfEntries;
                ...
                writeHeader(outBuffer, op_code_, returncode, totalMessageSize, numOfEntries, &bufferNext);
		...
		SYNDICATEFS_DATA->stats->leave( STAT_OPEN, rc );
		...
		if(returncode == 0) {
		    toWriteSize += 4 + SIZE_IPCFILEINFO;
                ...
                writeHeader(outBuffer, op_code_, returncode, 4 + SIZE_IPCFILEINFO, 1, &bufferNext);
		if(returncode == 0) {
                    outBuffer = bufferNext;
		...
		logmsg( SYNDICATEFS_DATA->logfile, "syndicateipc_create( %s, %o, %p )\n", path, mode );
		...
		// get status
		if(rc == 0) {
		    logmsg( SYNDICATEFS_DATA->logfile, "syndicateipc_fgetattr( %s )\n", path );
		    ...
		    rc = fs_entry_fstat( SYNDICATEFS_DATA->core, fh, &statbuf );
		...
		// close
		if(rc == 0) {
		    logmsg( SYNDICATEFS_DATA->logfile, "syndicateipc_release( %s )\n", path );
		    ...
		    rc = fs_entry_close( SYNDICATEFS_DATA->core, fh );
		    if( rc != 0 ) {
		...
		//struct fs_file_handle* fh = (struct fs_file_handle*)fi->fh;
		struct fs_file_handle* fh = (struct fs_file_handle*)fi.handle;
		//ssize_t returnsize = fs_entry_read( SYNDICATEFS_DATA->core, fh, buf, size, offset );
		ssize_t returnsize = fs_entry_read( SYNDICATEFS_DATA->core, fh, buffer, size, fileoffset );
		...
		//if( returnsize < (signed)size ) {
		    //memset( buf + returnsize, 0, size - returnsize );
		//}
   
		if( conf->debug_read )
	            logmsg( SYNDICATEFS_DATA->logfile, "syndicateipc_read rc = %ld\n", returnsize );
   
		SYNDICATEFS_DATA->stats->leave( STAT_READ, (returnsize >= 0 ? 0 : returnsize) );
		///////////

                int returncode = (int)returnsize;
                int toWriteSize = 16;
		if(returncode == 0) {
		    toWriteSize += 4 + (int)returnsize;
		}
                data_out_ = new char[toWriteSize];
                char* outBuffer = data_out_;
                char* bufferNext;

                writeHeader(outBuffer, op_code_, returncode, 4 + (int)returnsize, 1, &bufferNext);
		if(returncode == 0) {
                    outBuffer = bufferNext;
                    writeBytes(outBuffer, buffer, (int)returnsize, &bufferNext);
		}
        ...
        case OP_WRITE_FILE_DATA:
            {
                ...
                // call
		///////////
		//logmsg( SYNDICATEFS_DATA->logfile, "syndicateipc_write( %p, %ld, %ld, %p )\n", rawData, rawDataSize, fileoffset, fi->fh );
		logmsg( SYNDICATEFS_DATA->logfile, "syndicateipc_write( %p, %ld, %ld, %p )\n", rawData, rawDataSize, fileoffset, fi.handle );
		...
		ssize_t returnsize = fs_entry_write( SYNDICATEFS_DATA->core, fh, rawData, rawDataSize, fileoffset );
   
		//SYNDICATEFS_DATA->stats->leave( STAT_WRITE, (rc >= 0 ? 0 : rc)  );
		SYNDICATEFS_DATA->stats->leave( STAT_WRITE, (returnsize >= 0 ? 0 : returnsize)  );
        ...
        case OP_FLUSH:
            {
                ...
                char* bytes_ptr;
                IPCFileInfo fi;
                readFileInfo(message_, &fi, &bytes_ptr);
        ...
        case OP_CLOSE_FILE_HANDLE:
            {
                ...
                // call
		///////////
		logmsg( SYNDICATEFS_DATA->logfile, "syndicateipc_release( %p )\n", fi );
    ...
    int writeHeader(char* buffer, int opcode, int returncode, int totalMsgSize, int totalNumOfMsg, char** bufferNext) {
        char* bytes_ptr = buffer;
        writeIntToBuffer(bytes_ptr, opcode);
        bytes_ptr += 4;
        
        writeIntToBuffer(bytes_ptr, returncode);
        bytes_ptr += 4;

        writeIntToBuffer(bytes_ptr, totalMsgSize);
        bytes_ptr += 4;

        writeIntToBuffer(bytes_ptr, totalNumOfMsg);
        bytes_ptr += 4;
    ...
    int readPath(const char* msgFrom, char* outPath, char** msgNext) {
        ...
        int msgLen = getIntFromBytes(bytes_ptr);
        bytes_ptr += 4;
    ...
    int writePath(char* buffer, const char* path, int pathLen, char** bufferNext) {
        char* bytes_ptr = buffer;
        writeIntToBuffer(bytes_ptr, pathLen);
        bytes_ptr += 4;
    ...
    int readFileInfo(const char* msgFrom, IPCFileInfo* outFileInfo, char** msgNext) {
        ...
        outFileInfo->handle = getLongFromBytes(bytes_ptr);
    ...
    int writeFileInfo(char* buffer, const IPCFileInfo* inFileInfo, char** bufferNext) {
        char* bytes_ptr = buffer;
        writeIntToBuffer(bytes_ptr, 8);
        bytes_ptr += 4;
        
        writeLongToBuffer(bytes_ptr, inFileInfo->handle);
        bytes_ptr += SIZE_IPCFILEINFO;
    ...
    int writeStat(char* buffer, const IPCStat* stat, char** bufferNext) {
        char* bytes_ptr = buffer;
        writeIntToBuffer(bytes_ptr, SIZE_IPCSTAT);
        bytes_ptr += 4;
        
        writeIntToBuffer(bytes_ptr, stat->st_mode);
        bytes_ptr += 4;
        writeIntToBuffer(bytes_ptr, stat->st_uid);
        bytes_ptr += 4;
        writeIntToBuffer(bytes_ptr, stat->st_gid);
        bytes_ptr += 4;
        writeLongToBuffer(bytes_ptr, stat->st_size);
        bytes_ptr += 8;
        writeLongToBuffer(bytes_ptr, stat->st_blksize);
        bytes_ptr += 8;
        writeLongToBuffer(bytes_ptr, stat->st_blocks);
        bytes_ptr += 8;
        writeLongToBuffer(bytes_ptr, stat->st_atim);
        bytes_ptr += 8;
        writeLongToBuffer(bytes_ptr, stat->st_mtim);
        bytes_ptr += 8;
    ...
    int readLong(const char* msgFrom, long long int* outLong, char** msgNext) {
        ...
        *outLong = getLongFromBytes(bytes_ptr);
        bytes_ptr += 8;
    ...
    int writeLong(char* buffer, long long int value, char** bufferNext) {
        ...
        writeLongToBuffer(bytes_ptr, value);
        bytes_ptr += 8;
    ...
    int readInt(const char* msgFrom, int* outInt, char** msgNext) {
        ...
        *outInt = getIntFromBytes(bytes_ptr);
        bytes_ptr += 4;
    ...
    int writeInt(char* buffer, int value, char** bufferNext) {
        char* bytes_ptr = buffer;
        writeIntToBuffer(bytes_ptr, 4);
        bytes_ptr += 4;
        
        writeIntToBuffer(bytes_ptr, value);
        bytes_ptr += 4;
    ...
    int writeBytes(char* buffer, const char* bytes, int byteLen, char** bufferNext) {
        char* bytes_ptr = buffer;
        writeIntToBuffer(bytes_ptr, byteLen);
        bytes_ptr += 4;
...
private:
    ...
    char data_in_[max_length];
    char* data_out_;

    int op_code_;
    int total_msg_size_;
    int num_messages_;
    enum { max_header_length = 12 }; // int * 3
    char header_[max_header_length];
    int header_offset_;

    char* message_;
    int message_offset_;

    enum { stage_read_header = 0,
        stage_read_data = 1,
    };
    int stage_;    
};
