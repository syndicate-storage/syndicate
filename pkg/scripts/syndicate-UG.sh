...
PKG_INSTALL=$PKG_ROOT/out/syndicate-UG/usr
...
fpm --force -s dir -t rpm -v $PKG_VERSION -n $PKG_NAME $DEPARGS -d "libmicrohttpd > 0.9" -C $PKG_BUILD -p $PKG_PKGOUT --license proprietary --vendor PlanetLab --description "Syndicate User Gateways" $(ls $PKG_BUILD)
