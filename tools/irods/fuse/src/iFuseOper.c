...
irodsMknod (const char *path, mode_t mode, dev_t rdev)
{
    ...
    logmsg( LOGCTX, "irodsMknod(%s, %o, %lX)", path_hash, mode, rdev );
...
irodsSymlink (const char *to, const char *from)
{
    ...
    log_hash_path( LOGCTX, to, path_hash_to );
    log_hash_path( LOGCTX, from, path_hash_from );
...
irodsLink (const char *from, const char *to)
{
    ...
    logmsg( LOGCTX, "irodsLink(%s, %s)", path_hash_from, path_hash_to );
    logmsg( LOGCTX, "irodsLink(%s, %s) rc = %d\n", path_hash_from, path_hash_to, 0 );
    return (0);
}
