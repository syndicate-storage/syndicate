...
def create_and_activate_user( client, user_email ):
    ...
    try:
        ...
        new_user = client.create_user( user_id, user_openid_url, user_activate_pw, is_admin=False, max_UGs=1100, max_RGs=10 )
    except Exception, e:
...
#-------------------------------
def ensure_user_exists_and_has_credentials( user_email ):
    """ 
    ...
    if new_user is not None:
         try:
            rc = save_syndicate_principal( user_email, new_user['signing_public_key'], new_user['signing_private_key'] )
