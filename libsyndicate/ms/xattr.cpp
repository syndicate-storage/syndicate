...
int ms_client_getxattr( struct ms_client* client, uint64_t volume_id, uint64_t file_id, int64_t xattr_nonce, char const* xattr_name, char** xattr_value, size_t* xattr_value_len ) {
   ...
   getxattr_url = ms_client_getxattr_url( client->url, volume_id, ms_client_volume_version( client ), ms_client_cert_version( client ), file_id, xattr_name );
   ...
   if( rc != 0 ) {
      ...
      // get the xattr 
      val = SG_CALLOC( char, reply.xattr_value().size() );
      if( val == NULL ) {
         return -ENOMEM;
      }
      
      memcpy( val, reply.xattr_value().data(), reply.xattr_value().size() );
...
// TODO: get all xattrs, their values, and the coordinator's signature.
// this method should only be called by the coordinator when sync'ing itself with the MS, in which case, it needs to validate the signature.
...
int ms_client_listxattr( struct ms_client* client, uint64_t volume_id, uint64_t file_id, char** xattr_names, size_t* xattr_names_len ) {
   ...
   listxattr_url = ms_client_listxattr_url( client->url, volume_id, ms_client_volume_version( client ), ms_client_cert_version( client ), file_id );
