/*
 * JSFSPath class for JSyndicateFS
 */
 ...
 * @author iychoi
 */
public class JSFSPath implements Comparable {
     ...
     */
    public JSFSPath(String parent, String child) {
        this(new JSFSPath(parent), new JSFSPath(child));
    }
    
    public JSFSPath(JSFSPath parent, String child) {
        this(parent, new JSFSPath(child));
    }
    
    public JSFSPath(String parent, JSFSPath child) {
        this(new JSFSPath(parent), child);
    }
    
    public JSFSPath(JSFSPath parent, JSFSPath child) {
        if (parent == null)
    ...
    public JSFSPath getParent() {
        ...
        if (lastSlash == -1) {
            return new JSFSPath(createPathUri(this.uri.getScheme(), this.uri.getAuthority(), "."));
        } else if (lastSlash == 0) {
            return new JSFSPath(createPathUri(this.uri.getScheme(), this.uri.getAuthority(), "/"));
        } else {
            String parent = path.substring(0, lastSlash);
            return new JSFSPath(createPathUri(this.uri.getScheme(), this.uri.getAuthority(), parent));
        }
    }
    ...
    public JSFSPath[] getAncestors() {
        ArrayList<JSFSPath> ancestors = new ArrayList<JSFSPath>();
        
        JSFSPath parent = getParent();
        while(parent != null) {
        ...
        JSFSPath[] ancestors_array = new JSFSPath[ancestors.size()];
    ...
    public boolean equals(Object o) {
        if (!(o instanceof JSFSPath))
            return false;
        
        JSFSPath other = (JSFSPath) o;
